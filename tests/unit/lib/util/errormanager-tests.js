...
YUI.add('errormanager-tests', function(Y) {
    ...
    // suite.add(new Y.Test.Case({
    //     "errorCheck() should check environment": function(){
    //         mocks.invokeCount = 0;
    //         mocks.message = undefined;
    //         dimensions = JSON.parse(JSON.stringify(origDim));
    //         dimensions[0].dimensions[0].environment.qa1 = {};
    ...
    //         Y.Assert.areSame(undefined, mocks.message);
    //     }
    // }));
    ...
    suite.add(new Y.Test.Case({
        "errorCheck() should exit if enviroment is missing in dimensions file": function(){
            ...
            msg[1000] = {
                name: "EDIMENVTEST",
                text: 'The settings group {0} is missing. Please add environment "{1}" to dimensions file "{2}".'
            };
            try {
                em.errorCheck("ArrowSetup", {
                    mock : mocks,
                ...
                Y.Assert.areSame(
                    '1000 (EDIMENVTEST) The settings group "environment:qa1" is missing. Please add environment "qa1" to dimensions file "./config/dimensions.json".',
                    mocks.message
                );                
    ...
    // suite.add(new Y.Test.Case({
    //     "errorCheck() should exit if dimensions content has error": function(){
    //         mocks.invokeCount = 0;
    //         mocks.message = undefined;
    //         dimensions = JSON.parse(JSON.stringify(origDim));
    //         args = JSON.parse(JSON.stringify(origArgv));
    //         args.dimensions = "error";
    //         msg[1001] = {
    //             name: "ECHKDIMTEST",
    //             text: 'Please check dimensions file {0}. Wrong dimensions format : {1}',
    //             0 : "Path is not defined",
    ...
    //         finally {
    //             Y.Assert.areSame(
    //                 '1001 (ECHKDIMTEST) Please check dimensions file error. Wrong dimensions format : Unexpected token e',
    //                 mocks.message
    //             );
    //             Y.Assert.areSame(1, mocks.invokeCount);
    //         }
