...
Arrow.prototype.runController = function (controllerName, testConfig, testParams, driver, callback) {
    ...
    self.logger.info("Arrow.prototype.runController --------- ");
    if (!testConfig) {
        testConfig = {};
    }
    if (!testParams) {
    ...
    testWaitingTimeout = testParams.testTimeOut || self.config.testTimeOut || 120000;
    //setting callback
    ...
    self.controllerCallbackFlag = true;
    function controllerExecuteCallback (error, data) {
        self.logger.info("controllerExecuteCallback!!!!");
        var lastError = error;
        self.logger.info("clearInterval:"+JSON.stringify(self.interval));
        clearInterval(self.interval);
        delete self.interval;
        if (self.controllerCallbackFlag !== undefined) {
            delete self.controllerCallbackFlag; // make sure we only kill nodeChild once
            if (driver.nodeChild) {
                try {
                    self.logger.info("kill nodeChild !!!!");
                    driver.nodeChild.kill();
                } catch (e) {
                }
            }
        }
        controllerCallback (error, data);
    }

    self.logger.debug("Reset timer - set timeout " + testWaitingTimeout + " ms");
    if (self.interval) {
        self.logger.info("clearInterval:"+JSON.stringify(self.interval));
        clearInterval(self.interval);
        delete self.interval;
    ...
    self.interval = setInterval(function () {
        ...
        clearInterval(self.interval);
        delete self.interval;
        controllerExecuteCallback("Waiting controller execution timeout ("+testWaitingTimeout+" ms).");
    }, testWaitingTimeout);

    function controllerCallback (error, data) {

        var errorMsg;
        self.logger.debug("controllerCallback error is " + error);
        if (error) {
...
//            self.logger.debug("controllerCallback errorMsg is " + errorMsg);
//            self.logger.debug("testParams.test is "+JSON.stringify(testParams.test));
//            self.logger.debug("testParams.scenario is "+JSON.stringify(testParams.scenario));
//            self.logger.debug("testParams is "+JSON.stringify(testParams));
...
//                driver.reports.addReport({"error" : errorMsg, "controller" : controllerName, "params" : testParams});
        ...
        self.logger.debug("callback from arrow.js "+callback);
    ...
    if (driver.getWebDriver() && driver.getWebDriver().session_) {
        ...
        self.logger.info("executing controller for non webdriver tests --------- , Controller: " + controllerName);
        controller.execute(controllerExecuteCallback);
    }
};
