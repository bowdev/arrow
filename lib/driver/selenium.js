...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    addAsyncScriptCallback = function (driverjs) {
        return ";(function (callback) {\n"+driverjs+"}(arguments[arguments.length - 1]));";
    };
...
//                self.logger.trace("Driver js: " + driverJs);
                callback(null, addAsyncScriptCallback(driverJs));
            });
        }
    } else {
        driverJs =
...
SeleniumDriver.prototype.executeTest = function (testConfig, testParams, callback, web_driver) {
    ...
    //         setTimeout(collectTestReport, self.attemptInterval);
    //     });
        webdriver.manage().timeouts().setScriptTimeout(self.args.testTimeOut||self.config.testTimeOut||30000).then(function(){
            webdriver.executeAsyncScript(driverJs).then(function () {
                setTimeout(collectTestReport, self.attemptInterval);
            }).then(null, function (err) {
                self.logger.debug("initializing test error: "+err);
                setTimeout(collectTestReport, self.attemptInterval);
            });
        });
    }
