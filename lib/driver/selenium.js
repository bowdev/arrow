...
function replaceTestParams(self, feature, params) {
    var newFeature = feature;
    var modifier = "gi";
    self.logger.info("test params are "+JSON.stringify(params));
    for (var param in params) {
        //self.logger.info("test param "+param+" is "+params[param]);
        var regex = new RegExp("\\$\\{"+param+"\\}\\$",modifier);
        newFeature = feature.replace(regex,params[param]);
        feature = newFeature;
    }
    return newFeature;
}
...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            ...
            if (".feature"==path.extname(testJs)) {
                self.logger.info("Loading feature: " + resolvePath(testJs));
                testFeature += fs.readFileSync(resolvePath(testJs), "utf-8");
                testFeature = replaceTestParams(self, testFeature, testParams);
                testFeaturePair.push([resolvePath(testJs),testFeature]);
                if (typeof testParams.step === "string" && testParams.step.length > 0) {
                    var stepFiles = testParams.step.split(","), index;
                    testSteps = "[";
                    for (index in stepFiles) {
                        if (index>0) {
                            testSteps += ",";
                        }
                        testSteps += fs.readFileSync(resolvePath(stepFiles[index]), "utf-8");
                    }
                    testSteps += "]";
                }
                else {
                    self.logger.info ("test parameter 'step' is empty or not found.");
                }
                testCucumber += fs.readFileSync(require.resolve("../../ext-lib/cucumber.js"), "utf-8");
            } else {
                if (testJs) {
    ...
    trunkdriverJs =
        "ARROW = {};" +
            ...
            "ARROW.featureFiles = " + JSON.stringify(testFeaturePair) + ";" +
            "ARROW.testSteps = " + testSteps + ";" +
            "ARROW.CucumberJS = function() { " + testCucumber + " };" +
