...
SeleniumDriver.prototype.executeTest = function (testConfig, testParams, callback, web_driver) {
    ...
    function initTest() {
        var checkResult = function () { setTimeout(collectTestReport, self.attemptInterval);},
        checkLog = function(log) {self.logger.debug("ARROW.consoleLog is "+log);};

        webdriver.manage().timeouts().setScriptTimeout(self.attemptInterval*2);
        webdriver.executeAsyncScript(
            "var callback = arguments[arguments.length - 1]; window.setTimeout(function(){return callback(ARROW.consoleLog);},"+
            self.attemptInterval*2
            +"); try {window.setTimeout(function(){"+
            driverJs
            +"},0);} catch(e) {}").then(checkLog).then(checkResult);
    }
