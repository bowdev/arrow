...
SeleniumDriver.prototype.start = function (callback) {
    ...
    self.logger.info("finish webdriver loading");
    self.logger.info("session id is " + self.sessionId);
    ...
    interval = setInterval(function() {
        try {
            // make sure the webdriver session is active and ready to serve testing page.
            self.webdriver.getCurrentUrl().then(function (url) {
                self.logger.debug("webdriver is ready");
...
SeleniumDriver.prototype.stop = function (callback, error) {
    ...
    if (self.sessionId) {
        if (callback) {
            self.logger.error("sessionId:" + this.sessionId + " error: " + error);
            callback(null);
        }
    } else {
        // we were not given a session id
        if (self.webdriver && self.webdriver.session_ !== undefined) {
//            if (callback) {
//                self.logger.error("with webdriver:" + error);
//                callback(error);
//            }
            // self.webdriver.quit();
                self.webdriver.quit().then(function () {
                    delete self.webdriver;
                    if (callback) {
                        if (error) {
                            self.logger.error("within webdriver quit:" + error);
                        }
                        callback(error);
                    }
                }).then(null, function () {
                    delete self.webdriver;
                    if (callback) {
                        self.logger.error("within webdriver error callback:" + error);
                        callback(error);
                    }
                });
        } else {
            if (callback) {
                self.logger.error("without webdriver:" + error);
                callback(error);
            }
        }
    }
...
SeleniumDriver.prototype.navigate = function (page, callback) {
    ...
    if (self.webdriver === undefined || self.webdriver.session_ === undefined) {
        callback();
        return;
    }
    ...
    if (false === url) {
        ...
        self.webdriver.get(url).then(function () {
            callback();
        }, function(err) {
...
SeleniumDriver.prototype.executeTest = function (testConfig, testParams, callback, web_driver) {
    ...
    testJs = testParams.test;
    page = testParams.page;
    if (web_driver && web_driver.session_) {
        logger.info("webdriver session_ is " + JSON.stringify(web_driver.session_));
    } else if (self.webdriver && self.webdriver.session_) {
        logger.info("webdriver session_ is " + JSON.stringify(self.webdriver.session_));
    } else {
        // logger.info("Webdriver session is timeout and end. Do not execute tests.");
        self.errorCallback(logger, 'Webdriver session is timeout and end. Do not execute tests.', callback);
        // callback("Webdriver session is timeout and end. Do not execute tests.");
        return;
    }
    webdriver = web_driver || self.webdriver;
    ...
    this.createDriverJs(testParams, function (err, driverjs) {
        ...
        if (webdriver.session_) {
            ...
            self.errorCallback(logger, "Webdriver session is not found, check your selenium config", callback);
            return;
        }
