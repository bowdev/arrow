...
DefaultController.prototype.execute = function (callback) {
    var self = this,
        timeout = self.testParams.testWaitingTimeout || self.testConfig.testWaitingTimeout || 60000,
        checkCallback = function(errorMsg) {
            if (self.callback) {
        ...
        timeoutCallback = function() {
            setTimeout(function() {
                checkCallback("Waiting controller execution timeout ("+timeout+" ms).");
            }, timeout);
        };
    ...
    if (self.testParams.scenario) {
        self.executeScenario(checkCallback);
    } else if (self.testParams.action) {
        self.driver.executeAction(self.testConfig, self.testParams, checkCallback);
    } else {
        if (!self.testParams.test && self.testParams.page) {
            self.driver.navigate(self.testParams.page, checkCallback);
        } else if (self.testParams.test) {
            self.driver.executeTest(self.testConfig, self.testParams, checkCallback);
        } else {
            checkCallback("Neither test nor page is specified");
        }
    }
};
