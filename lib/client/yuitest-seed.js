...
// console.log("add try catch to ARROW.onSeeded");
ARROW.onSeededOrig = ARROW.onSeeded;
ARROW.onSeeded = function () {
    var errorMessage = "Exception:";
    try {
        // console.log("Begin onSeeded ");
        ARROW.onSeededOrig();
    } catch(e) {
        if (typeof e === "string") {
            errorMessage += e;
        ...
        ARROW.testReportJson.testCase.test.message = errorMessage;
        ARROW.testReport = JSON.stringify(ARROW.testReportJson);
    } finally {
        // console.log("End onSeeded ");
    }
};
...
(function () {
    ...
    function onYUIAvailable() {
        ...
        if (!YUI.ARROWYuiAddFunc) {
            YUI.ARROWYuiAddFunc = YUI.add;
        }
        ...
        YUI.add = function (name, fn, version, meta) {
            if (ARROW.testReportJson) {
                ARROW.testReportJson.name = name;
            }
            YUI.ARROWYuiAddFunc(name, fn, version, meta);
